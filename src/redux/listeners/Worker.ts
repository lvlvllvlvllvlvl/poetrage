...
import { Inputs } from "./calculateProfits";
...
self.onmessage = (e: MessageEvent<{ inputs: Inputs; cancel: URL }>) => {
  const [
    gems,
    currencyMap,
  ...
  if (gemInfo.status === "done") {
    ...
    for (const gem of result) {
      ...
      if (!gem.Corrupted && gem.Type !== "Awakened" && gemInfo.value.weights[gem.baseName]) {
        const weights = gemInfo.value.weights[gem.baseName].filter(({ Type }) => Type !== gem.Type);
